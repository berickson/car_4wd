...
void Mpu9150::setup() {
  ...
  if (devStatus == 0) {
    ...
    Serial.print(F("Enabling interrupt detection (Arduino external interrupt ")); Serial.println(digitalPinToInterrupt(INTERRUPT_PIN));
    ...
    Serial.println("Got first interrupt!");
...
void Mpu9150::execute(){
  ...
  if (/*(mpuIntStatus & 0x10) || */fifoCount >= 1024) {
    ...
    // bke modified from while to if/return, maybe was hanging
    ...
    if(!initialReading) {
      ...
      groundAngle = -rads2degrees(normal_angle(qdiff,gravity));
    }
  }
}
